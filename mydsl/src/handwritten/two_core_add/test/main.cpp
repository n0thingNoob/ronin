// main.cpp — phase 2 step 1 test driver for two_core_add
//
// Generates deterministic A, B, runs the device path (2-core dataflow
// producer/consumer with NOC L1-to-L1 transfer of pa, pb), computes a CPU
// reference for C = A + B, reports max diff. Returns 0 on PASS.
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        // Use values exactly representable in bf16 so the float reference
        // matches the bf16-domain device add bit-for-bit.
        a[i] = float(i % 16);
        b[i] = float(((i * 3) % 16) + 1);
    }
    ...
    if (first_bad >= 0) {
        printf("  first_bad: i=%d c=%g exp=%g\n",
            first_bad, c[first_bad], expected_bf16[first_bad]);
        // Bad index histogram by tile (1 tile = 1024 elements).
